...
import inkex
from inkex import ( TextElement, Rectangle )
...
class CalendarTemplate(inkex.EffectExtension):
    ...
    def add_arguments(self, pars):
        ...
        pars.add_argument(
            ...
            dest="font_size_day",
            default="7mm",
            help="Font size for the day strings.",
        )
        pars.add_argument(
            "--font-size-day-name",
            type=str,
            dest="font_size_day_name",
            default="5mm",
            help="Font size for the day strings.",
        )
        pars.add_argument(
    ...
    def validate_options(self):
        ...
        self.options.page_margin = self.svg.unittouu(self.options.page_margin)
        self.options.font_size_day = self.svg.unittouu(self.options.font_size_day)
        self.options.font_size_day_name = self.svg.unittouu(self.options.font_size_day_name)
    ...
    def calculate_size_and_positions(self):
        ...
        self.style_day_name = {
            "font-size": self.options.font_size_day_name,
            "font-family": self.options.color_day_name,
            "text-anchor": "middle",
            "text-align": "center",
            "fill": self.options.color_day_name,
        }
    ...
    def write_month_header(self, g, m):
        ...
        try:
            g.add(TextElement(**txt_atts)).text = unicode(
                self.options.month_names[m - 1] + " " + str(self.options.year), self.options.input_encode
            )
        except:
        ...
        for wday in day_names:
            txt_atts = {
                ...
                "x": str(self.day_w * week_x + self.day_w/2),
                "y": str(self.day_h-self.options.font_size_day*0.15),
            }
            try:
    ...
    def create_month(self, m):
        ...
        txt_atts = {
            "transform": "translate("
            + str(self.options.page_margin)
            + ","
            # + str((self.day_h * 4) + (self.month_h * self.month_y_pos))
            + str(self.doc_h - (self.day_h*7 + self.options.page_margin))
            + ")",
        ...
        for week in cal:
            ...
            for day in week:
                ...
                txt_atts = {
                    "style": str(inkex.Style(style)),
                    "x": str(self.day_w * week_x + self.options.font_size_day * 0.15),
                    "y": str(self.day_h * (week_y + 2)-(self.day_h-self.options.font_size_day)),
                }
                text = None
                ...
                if text:
                    ...
                    day_rect = Rectangle(
                                x=str(self.day_w * week_x),
                                y=str(self.day_h * (week_y + 1)),
                                height= str(self.day_h),
                                width = str(self.day_w))
                    day_rect.style = {  'stroke'        : '#000000',
                                        'stroke-width'  : '0.1',
                                        'fill'          : 'none'
                                    }
                    gdays.add(day_rect)
    ...
    def draw_SVG_square(self):
        
        rect = Rectangle(
            x=str(self.options.page_margin),
            y=str(self.doc_h /2),
            height= str((self.doc_h/2)-self.options.page_margin),
            width = str((self.doc_w)-2*self.options.page_margin))
